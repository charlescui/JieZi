...
-(void)commonInitWithImage:(UIImage *)image andTitle:(NSString *)aTitle andFrame:(CGRect) frame{
    ...
    if (image) {
        ...
        if (([aTitle length] > 0) && (image == nil)) {
            title = [[UILabel alloc] initWithFrame:imageView.frame];
            title.text = aTitle;
            title.backgroundColor = [UIColor clearColor];
            title.font = [UIFont fontWithName:@"Avenir Next" size:46];
            title.textAlignment = NSTextAlignmentCenter;
            title.textColor = [UIColor whiteColor];
        }
    }
...
-(instancetype)initWithItemImage:(NSArray *)items andTitles:(NSArray *)titleArray andActionArray:(NSArray *)actionArray addToViewController:(UIViewController*)sender {
    ...
    if (self) {
        ...
        UIView *snapshotView = [sender.view snapshotViewAfterScreenUpdates:YES];
        UIImage *screenShot = [snapshotView screenshot];
        ...
        }else if([titleArray count] == [actionArray count]){
            int index = 0;
            for (NSString *label in titleArray) {
                BTPopUpItemView *item = [[BTPopUpItemView alloc] initWithImage:nil title:label action:[actionArray objectAtIndex:index]];
                [itemArray addObject:item];
                index++;
            }
        }
